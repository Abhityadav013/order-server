  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-lg transition-all duration-300 group">
      <div className="flex flex-col sm:flex-row">
        {/* Image */}
        <div className="relative w-full h-24 sm:w-32 sm:h-24 flex-shrink-0 overflow-hidden">
          <Image
            src={item.imageURL}
            alt={item.name}
            width={128}
        ...
        <div className="flex-1 p-2 sm:p-4">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
            ...
            <div className="flex items-center justify-between sm:flex-col sm:items-end gap-3">
              <span className="sm:text-sm font-bold text-gray-900">
                €{item.price.toFixed(2)}
              </span>
              <button
                onClick={() => onAddToBasket(item)}
                disabled={!isRestaurantOpen}
                className={`flex items-center gap-2 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg transition-all duration-200 font-medium shadow-sm ${
                  isRestaurantOpen
